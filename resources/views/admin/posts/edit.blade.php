                ...
                <div class="p-10">
                    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="title" id="title" value="{{ $post->title }}" class="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/5 transition shadow-sm font-bold text-gray-900" required>
                        </div>

                        <div class="space-y-2">
                            <label for="content" class="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-1">Narrative Content</label>
                            <textarea name="content" id="content" rows="12" class="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/5 transition shadow-sm" required>{{ $post->content }}</textarea>
                        </div>
                            ...
                            <div class="space-y-2">
                                ...
                                @if($post->featured_image)
                                    <div class="mb-4 relative group">
                                        <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Current Image" class="h-32 w-full object-cover rounded-xl shadow-md">
                                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center rounded-xl">
                                            <span class="text-[9px] text-white font-bold uppercase tracking-widest">Replace Below</span>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-2">
                                ...
                                <select name="status" id="status" class="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm focus:border-cyan-500 transition shadow-sm">
                                    <option value="draft" {{ $post->status == 'draft' ? 'selected' : '' }}>Draft (Internal Only)</option>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="date" name="published_at" id="published_at" value="{{ $post->published_at ? $post->published_at->format('Y-m-d') : '' }}" class="w-full bg-white border border-gray-100 rounded-2xl px-6 py-4 text-sm focus:border-cyan-500 transition shadow-sm">
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" name="price" id="price" value="{{ $post->price }}" class="w-full bg-white border border-gray-100 rounded-2xl pl-16 pr-6 py-4 text-sm font-bold text-cyan-600 focus:border-cyan-500 transition shadow-sm" required>
                            </div>
                        </div>

                        <div class="pt-6 border-t border-gray-50 flex items-center justify-between">
                            <a href="{{ route('admin.posts.index') }}" class="text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-gray-600 transition">Discard Changes</a>
